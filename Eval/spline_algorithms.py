...
axis[0, 0].plot(x_middle, y_middle,
                label='Centerline (provided)', color='gray')
axis[0, 0].grid()
...
for index in range(len(x_middle)):
    ...
    parallel_slope = (-1) / derivative if derivative != 0 else 1
    ...
    tangent_vector = np.array([x_middle[(index + 1) % len(x_middle)] -
                               x_middle[index], (y_middle[(index + 1) % len(x_middle)] - y_middle[index])])
...
indexes = list(range(len(x_left)))
# pick random 300 elements, retaining the order
random_indexes_left = sample(indexes, 300)
...
# todo: condition may not always hold
...
axis[1, 1].set_title(
    f'Restored track sides via [0-1) spline interpolation. {t2 - t1} s')
