...
for index in range(len(left_points)):
    ...
    a = (right_x_spline.get(right_points[index]),
         right_y_spline.get(right_points[index]))
    b = (left_x_spline.get(left_points[index - indices_difference]),
         left_y_spline.get(left_points[index - indices_difference]))
    c = (left_x_spline.get(left_points[index]),
    ...
    x_tangent_vector_at_a = (right_x_spline.get(right_points[(
        index + 1) % len(left_points)] - right_x_spline.get(right_points[index - 1]))) / 2 / dt_right
    y_tangent_vector_at_a = (right_y_spline.get(right_points[(
        index + 1) % len(right_points)]) - right_y_spline.get(right_points[index - 1])) / 2 / dt_right

    perpendicular_vector_at_a = utility_fncs.left_side_perpendicular_vector(
        x_tangent_vector_at_a, y_tangent_vector_at_a)
    unit_perpendicular_vector_at_a = utility_fncs.unit_vector(
        perpendicular_vector_at_a[0], perpendicular_vector_at_a[1])

    centerline_x[index] = a[0] + \
        unit_perpendicular_vector_at_a[0] * track_width_at_a / 2
    centerline_y[index] = a[1] + \
        unit_perpendicular_vector_at_a[1] * track_width_at_a / 2

    if not index % 50:
        # print(f'a_b: {a_b}, a_c: {a_c}, b_c: {b_c}')
        print(
            f'Track width: {track_width_at_a}, a_c:{a_c}, triangle_area: {triangle_area}')
