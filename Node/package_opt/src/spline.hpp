...
namespace opt::spline {
    ...
    class NaturalSpline {
    public:
        explicit constexpr NaturalSpline(std::span<T> t, std::span<const T> y);

        [[nodiscard]] constexpr
        T get_at(T arg) const noexcept;

        [[nodiscard]] constexpr
        std::vector<T> get_range(T start, T stop, T increment) const;

        [[nodiscard]] constexpr
        std::vector<T> get_n_points(std::size_t points_count) const;

        [[nodiscard]] constexpr
        T get_derivative_at(T arg) const noexcept;

        [[nodiscard]] constexpr
        T get_second_derivative_at(T arg) const noexcept;
    ...
    NaturalSpline<T>::NaturalSpline(std::span<T> t, std::span<const T> y) {
        ...
        for (std::size_t index = 1; index < n; ++index) {
            alpha[index] = 3 / h[index] * (spline_fragments.at(index + 1).a - spline_fragments[index].a) -
                    3 / h[index - 1] * (spline_fragments.at(index).a - spline_fragments.at(index - 1).a);
        }
        std::vector<T> l(n + 1);
        std::vector<T> mu(n + 1);
        std::vector<T> z(n + 1);
        ...
        z[0] = 0;
        for (std::size_t index = 1; index < n; ++index) {
            l[index] = 2 * (spline_fragments[index + 1].x - spline_fragments[index - 1].x) - h[index - 1] * mu[index - 1];
    ...
    template<typename T>
    requires std::is_floating_point_v<T>constexpr std::vector<T>
    NaturalSpline<T>::get_range(T start, T stop, T increment) const {
        //the purpose of this function is to optimize generating a range of values by minimizing the amount of searches needed
        //start is inclusive, stop is exclusive
        opt_assert(increment > 0);
        opt_assert(start < stop);
        //find starting x: biggest x to satisfy x < arg

        const std::size_t required_vector_size{std::ceil((stop - start) / increment)};  //fixme: floor or ceil??
        std::vector<T> y_range(required_vector_size);
        ...
        while (start < stop) {
            ...
            y_range.at(vector_index++) = spline_fragments[spline_index].d * std::pow(delta_arg, 3) +
            ...
            start += increment;
            //check if the next spline fragment is now the biggest that start < spline_fragments
    ...
    template<typename T>
    requires std::is_floating_point_v<T>constexpr T NaturalSpline<T>::get_derivative_at(T arg) const noexcept {
    ...
    template<typename T>
    requires std::is_floating_point_v<T>constexpr T NaturalSpline<T>::get_second_derivative_at(T arg) const noexcept {
    ...
    template<typename T>
    requires std::is_floating_point_v<T>std::size_t NaturalSpline<T>::get_index(T arg) const noexcept{
